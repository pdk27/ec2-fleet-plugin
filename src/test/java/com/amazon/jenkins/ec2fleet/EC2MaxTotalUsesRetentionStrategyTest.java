...
public class EC2MaxTotalUsesRetentionStrategyTest {
    ...
    public void shouldNotTerminateWhenUsageIsGreaterThanOne() throws Exception {
        ...
        while (usageCount > -1) {
            ...
            r.jenkins.addNode(new EC2FleetNode("id", "test description", "fs", 1, null,
                    "label", null, new ArrayList<NodeProperty<?>>(), cloud.name, null, usageCount));

            final EC2FleetNode node = (EC2FleetNode) r.jenkins.getNodes().get(0);
            final EC2FleetNodeComputer computer = new EC2FleetNodeComputer(node);
